...
- روی Pull Request، شاخه `main` و تگ‌های انتشار، Build ویندوز و Unit Test اجرا می‌شود.
- Release فقط زمانی ساخته می‌شود که Commit تگ داخل تاریخچه `main` باشد.
- اکشن‌های GitHub با Commit SHA ثابت شده‌اند و Job ساخت فقط دسترسی خواندن دارد.
- کنار ZIP رسمی فایل SHA256 برای بررسی صحت دانلود منتشر می‌شود.

اجرای تست‌ها روی ویندوز:

`dotnet test DNSYar.Tests/DNSYar.Tests.csproj -c Release`

## امنیت بروزرسانی DNS

فهرست آنلاین فقط از آدرس HTTPS دریافت می‌شود. ورودی‌ها پیش از Merge از نظر IPv4 معتبر، ساختار JSON/TXT و تکراری نبودن بررسی می‌شوند. فقط منبعی را تنظیم کنید که به مالک آن اعتماد دارید؛ مالک منبع می‌تواند DNSهای پیشنهادی برنامه را تغییر دهد.

## مجوز

کد مخزن برای مشاهده عمومی است و فعلاً تحت مجوز «تمام حقوق محفوظ است» قرار دارد. مجوزهای مستقل فونت‌ها در `THIRD_PARTY_SOURCES.md` ثبت شده‌اند.
